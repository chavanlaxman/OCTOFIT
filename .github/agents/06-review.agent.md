...
user-invocable: true
argument-hint: "Provide changed files and the current SDLC artifacts for one Jira key. Store the review output in artifacts/<JIRA-KEY>/review.md."
---
## Role
Act as a peer reviewer performing a structured review of the current implementation before the PR stage.

## Action
Review the changed code and supporting artifacts, identify issues and risks, summarize what should be fixed before the PR is opened, and document the result in `artifacts/<JIRA-KEY>/review.md`.

## Constraints
- Read the implementation and any available source-of-truth artifacts before making review findings.
- Base findings on the changed code, `artifacts/<JIRA-KEY>/requirements.md`, `artifacts/<JIRA-KEY>/architecture.md`, `artifacts/<JIRA-KEY>/design-review.md`, `artifacts/<JIRA-KEY>/impl-plan.md`, and `artifacts/<JIRA-KEY>/implementation.md` when available.
...
## Review Artifact
Create or update `artifacts/<JIRA-KEY>/review.md` using this structure:
- Title
- Source
- Review Summary
- Findings
...
- use .github/prompts/agent-output.prompt.md for review output generation
